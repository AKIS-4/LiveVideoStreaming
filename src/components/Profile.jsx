...
const Profile = () => {
    ...
    const handleSubmit3 = async (e) => {
        e.preventDefault();
        let response = await fetch(`http://${import.meta.env.VITE_name}:3000/api/video/updatevideo`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ name: username + '_' + video2, tag: username + '_' + tag3 })
        });
        response = await response.json()
        setVideo2('')
        setTag3('')
    ...
    const handleSubmit4 = async (e) => {
        e.preventDefault();
        let response = await fetch(`http://${import.meta.env.VITE_name}:3000/api/tag/rmtag`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ name: user2, tag: username + '_' + tag4 })
        });
        response = await response.json()
        setUser2('')
        setTag4('')
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit4} className='flex justify-between shadow-2xl mb-10 rounded-3xl'>
                <div className="w-full max-w-2xl p-8 rounded-lg ">
                    <input
                        type="text"
                        value={user2}
